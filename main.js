...
const { normalizeLanguage: normalizePickerLanguage } = require('./electron/picker-i18n');
...
async function loadLocalizedPicker(pickerWindow, filename) {
  ...
  await pickerWindow.loadFile(path.join(__dirname, filename), { query: { lang: language } });
}
...
function createWindow () {
  ...
  ses.setDevicePermissionHandler(() => true);
    ...
    function openBlePicker() {
      ...
      void loadLocalizedPicker(blePickerWin, 'btPicker.html');
  ...
  // ⚑ vlastní volba portu
    ses.on('select-serial-port',
      ...
      void loadLocalizedPicker(picker, 'portPicker.html');
...
async function collectLegacyStorageOnce() {
  ...
  const legacyUrl = pathToFileURL(path.join(__dirname, 'index.html')).toString();
    ...
    try {
      migrationWindow = new BrowserWindow({
        ...
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
          preload: path.join(__dirname, 'electron', 'legacy-storage-preload.js'),
          sandbox: false,
        },
      });
...
async function startDesktopServer() {
  ...
  try {
    localServer = await startLocalServer({
      ideRoot: __dirname,
      simulatorRoot: path.join(__dirname, 'simulator_lite'),
